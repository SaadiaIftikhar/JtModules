classdef plotting
    ...
    properties (Constant)
        ...
        COLORBAR_POSITION_MAPPING = struct( ...
            'ul', [1-plotting.PLOT_HEIGHT, plotting.PLOT_WIDTH], ...
            'ur', [1-plotting.PLOT_HEIGHT, 1], ...
            'll', [0, plotting.PLOT_WIDTH], ...
            'lr', [0, 1] ...
        );
    ...
    methods (Static)
        ...
        function plot = create_histogram_plot(data, position, varargin)
            ...
            pos = plotting.PLOT_POSITION_MAPPING.(position);
            plot = struct( ...
                        'type', 'histogram', ...
                        'x', data, ...
                        'marker', struct( ...
        ...
        function plot = create_intensity_image_plot(image, position, varargin)
            ...
            ds_img = imresize(image, 1/plotting.IMAGE_RESIZE_FACTOR);
            dims = size(image);
            ds_dims = size(ds_img);

            pos = plotting.PLOT_POSITION_MAPPING.(position);
            col_pos = plotting.COLORBAR_POSITION_MAPPING.(position);
        ...
        function plot = create_mask_image_plot(mask, position, varargin)
            ...
            if any(colorscale_index)
                ...
                if n_objects == 1
                    ...
                    colorscale = plotting.create_colorscale('summer', n_objects);
        ...
        function plot = create_overlay_image_plot(image, mask, position, varargin)
            ...
            colorscale = plotting.create_colorscale('gray', clip_value);
            ...
            for i = 1:length(plots)
                if plot_positions{i} == 'ul'
                    layout.xaxis1 = struct( ...
                            'domain', [0, plotting.PLOT_WIDTH], ...
                            'anchor', 'y1' ...
                    );
                    layout.yaxis1 = struct( ...
                            'domain', [1-plotting.PLOT_HEIGHT, 1], ...
                            'anchor', 'x1' ...
                    );
                    if plot_is_image(i)
                ...
                elseif plot_positions{i} == 'ur'
                    layout.xaxis2 = struct( ...
                            'domain', [1-plotting.PLOT_WIDTH, 1], ...
                            'anchor', 'y2' ...
                    );
